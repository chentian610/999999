...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		initHeight:function(h){
			var self = this;
			var withoutSearchHeight = (self.box.offsetHeight - self.el.search.offsetHeight - h) + 'px';
			self.el.bar.style.height = withoutSearchHeight;
			self.el.inner.style.height = withoutSearchHeight;
			var barItemHeight = ((self.box.offsetHeight - self.el.search.offsetHeight - h) / (self.el.barItems.length+1)) + 'px';
		...
		search: function(keyword) {
			...
			liArray.forEach(function(item) {
				...
				if (item.classList.contains($.className('indexed-list-group'))) {
					...
					var tags1 = (item.getAttribute('data-tags1') || '').toLowerCase();
					var tags2 = (item.getAttribute('data-tags2') || '').toLowerCase();
					if (keyword && text.indexOf(keyword) < 0 &&
						value.indexOf(keyword) < 0 &&
						tags1.indexOf(keyword) < 0 &&
						tags2.indexOf(keyword) < 0 &&
