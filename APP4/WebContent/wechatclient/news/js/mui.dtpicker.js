...
(function($, document) {
	...
	var domBuffer = '<div class="mui-dtpicker" data-type="datetime">\
		<div class="mui-dtpicker-header">\
			...
			<button data-id="btn-ok" class="mui-btn mui-btn-green">确定</button>\
		</div>\
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		...
		_getDataReturn:function(d){
			if(d.y.value == '0'){
				...
				d.value = d.y.value + '-' + d.m.value ;
				d.text = d.y.text + '-' + d.m.text;
			}else{
				d.value = d.y.value + '-' + d.m.value + '-' + d.d.value;
				d.text = d.y.text + '-' + d.m.text + '-' + d.d.text;
			}
			return d;
		},
		...
		_createMonth: function(current) {
			...
			if (options.customData.m) {
				...
				if (options.totaLevel > 1) {
					if (ui.y.picker.getSelectedValue() == '0') {
						mArray.push({
							text: "全部",
							value: "0"
						});
		...
		_createDay: function(current) {
			...
			if (options.customData.d) {
				...
				if (options.totaLevel > 2) {
					if (ui.m.picker.getSelectedValue() == "0") {
						dArray.push({
							text: "全部",
							value: "0"
						});
					...
					if (ui.m.picker.getSelectedValue() != "0"&&ui.y.picker.getSelectedValue() != "0") {
						var maxDay = self.getDayNum(parseInt(ui.y.picker.getSelectedValue()), parseInt(ui.m.picker.getSelectedValue()));
						for (var d = 1; d <= maxDay; d++) {
							var val = self._fill(d);
							dArray.push({
