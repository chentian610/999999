...
;(function(root, factory) {
    ...
    jeDt.addDateTime = function(time,num,type,format){
        ...
        var tarr = jeDt.IsNum(time) ? nocharDate.match(ymdMacth) : time.match(ymdMacth), date = new Date(),
            ...
            tm3 = tarr[3] == undefined ? date.getHours() : parseInt('09'), tm4 = tarr[4] == undefined ? date.getMinutes() : parseInt('00'), tm5 = tarr[5] == undefined ? date.getMinutes() : parseInt(tarr[5]);
    ...
    jeDt.chooseDays = function() {
        jeDt.bind(jeDt.find(".jedaul li"), "click", function(ev) {
            ...
            document.querySelector(".jedaul .action").classList.remove('action');
            that.classList.add('action');
        });
    };
    //下拉选择年和月
