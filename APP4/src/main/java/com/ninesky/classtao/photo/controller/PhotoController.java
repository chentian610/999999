...
//        photoService.addInformation(photoList);
	...
	@ResultField(includes={"photo_id","student_id","photo_url","photo_resize_url","send_time","sender_id","version","point_praise","photo_comment_list"})
	 ...
	 * 用户添加评论
	 */
	@RequestMapping(value="/insertPhotoComment")
	public @ResponseBody Object insertPhotoComment(HttpServletRequest request){
		PhotoCommentVO vo = BeanUtil.formatToBean(PhotoCommentVO.class);
		photoService.insertPhotoComment(vo);
		return ResponseUtils.sendSuccess();
	}
	 ...
	 * 用户点赞
	 */
	@RequestMapping(value="/insertPhotoPointPraise")
	public @ResponseBody Object insertPhotoPointPraise(HttpServletRequest request){
		PhotoCommentVO vo = BeanUtil.formatToBean(PhotoCommentVO.class);
		photoService.insertPhotoPointPraise(vo);
		return ResponseUtils.sendSuccess();
	}
	 ...
	 * @return
	 */
	@RequestMapping(value="/getClassCircleList")
	@ResultField(includes={"school_id","class_id","team_type","add_date","photo_list","point_praise","point_praise_total","comment_list"})
	public @ResponseBody Object getClassCircleList(HttpServletRequest request){
		Map<String,String> paramMap = ActionUtil.getParameterMap();
		return photoService.getClassCircleList(paramMap);
	}
}
