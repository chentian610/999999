...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
       ...
       // linkContentHandler(editor, menu, $linkContent);
        ...
        // 增加到editor对象中
        editor.menus[menuId] = menu;
        function uploadPicture(uploadImgUrl){
            $('.wangeditor-menu-img-picture').on('click',function(){
                $("#upload_pic").click();
            });
            $('#upload_pic').unbind("change").on('change',function(){
                ...
                $.myajax({
                    ...
                    beforeSend: function () {
                       layer.msg('图片正在上传，请稍等！',{icon:0});
                    },
                    success: function (result) {
                        var item = result.result.data;
                        $('.wangEditor-txt').append('<img style="max-width:100%;" src="' + item[0].file_url + '"/>');
                        clearFile(uploadImgUrl);
                    }
                });
            });
        }
        function clearFile(uploadImgUrl) {
            ...
            $('#upload_pic').unbind("change").on('change',function(){
                ...
                $.myajax({
                    ...
                    success: function (result) {
                        var item = result.result.data;
                        $('.wangEditor-txt').append('<p></p><img style="max-width:100%;" src="' + item[0].file_url + '"/>');
                        clearFile(uploadImgUrl);
                    }
                });
            });
