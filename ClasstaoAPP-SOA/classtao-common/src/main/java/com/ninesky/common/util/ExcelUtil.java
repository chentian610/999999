...
//            throw new BusinessException("您导入的不是Excel文件格式，请确认！");
        /** 检查文件是否存在 */  
//        File file = new File(filePath);
//        if (file == null || !file.exists())
//        	throw new BusinessException("文件读取失败或者不存在，请确认！");
        return true;  
    }  
    ...
    private static <T> List<T> read(Workbook wb, HashMap<String,String> fieldMap, Class<T> clazz) throws InstantiationException, IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException, NoSuchFieldException
    {
        ...
        for (int c = 0; c < col_num; c++) {
            ...
            if (fieldMap.get(getCellValue(sheet.getRow(0), c))==null) break;//文件中有模版里没有的列字段
            fieldMap.put(c + "", fieldMap.get(getCellValue(sheet.getRow(0), c)));
        }
//        if (fieldMap.size()<size*2) throw new BusinessException("excel格式与模版不符！");
        /** 循环Excel的行 */  
        ...
        {  
            ...
            for (int c = 0; c < col_num; c++)  
            {
        		...
        		Method fieldSetMet = clazz.getDeclaredMethod(parGetName(fieldName,"set"),field.getType());
