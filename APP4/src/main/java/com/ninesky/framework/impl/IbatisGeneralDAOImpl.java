package com.ninesky.framework.impl;
...
public class IbatisGeneralDAOImpl implements GeneralDAO {
	...
	private static Logger logger = LoggerFactory.getLogger(IbatisGeneralDAOImpl.class);
	...
	public void setSqlSession(SqlSessionTemplate sqlMapClient) {
		...
		logger.debug("Mybatis数据库连接对象注入GeneralDAO完成");
	}
	...
	private BusinessException handException(Exception e) {
		...
		if (e instanceof DuplicateKeyException)
		{
			...
			int start = cause.indexOf("for key '");
			int end = cause.substring(start+9).indexOf("'");
			String indexName = cause.substring(start+9, start+9+end);
			return new BusinessException(2001,MsgService.getMsg("DB_"+indexName));
		} else if (e instanceof BadSqlGrammarException)
		{
			logger.error(((BadSqlGrammarException)e).getSQLException().getErrorCode()+"");
			logger.error(((BadSqlGrammarException)e).getSQLException().getMessage());
		}
		e.printStackTrace();
		return new BusinessException(2000,"数据异常，请刷新后重试...");
	}
